...
while True:
    #accept trava a exucução até q a requisição seja recebida
    client_connection, client_address = server_socket.accept()

    #a resposta esta em bytes , então utilizamos o decode para transformaar em string
    request = client_connection.recv(1024).decode()
    print('*'*100)
    print(request)

    route = extract_route(request)
    #essa proxima linha é uma vaiavel string com o caminho do diretorio onde o arquivo servidor.py esta localizado
